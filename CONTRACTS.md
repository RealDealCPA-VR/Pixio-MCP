...
## v1.1 addendum — any-LLM / local-model round (2026-07-11, supersedes conflicting text above)

1. **Schema descriptions:** every tool parameter is annotated `Annotated[T, Field(description="...")]` (`typing.Annotated` + `pydantic.Field` imported at module top level — FastMCP resolves string annotations with `inspect.signature(eval_str=True)`, so names must exist in module globals). Descriptions are short (≤120 chars), imperative, and include an example where useful (`model_id: "Model id from list_models, e.g. pixio/flux-1/schnell"`).
2. **Estimate=0 cap bypass fix:** in `resolve_estimate`, `estimatedCost == 0` is NOT a usable estimate — fall through to catalog cost. estimate 0 + catalog > 0 → `(catalog_cost, "catalog", warning noting the estimate was 0)`. estimate 0 + catalog == 0 → `(0, "catalog", None)` (genuinely free, e.g. video-ops). estimate 0 + catalog unknown → `(None, "unknown", warning)`.
3. **Input leniency:** `model_id`/`generation_id` are stripped of surrounding whitespace and backticks at tool entry (all tools that take them). `generate` and `estimate_cost` accept `params: dict | str` — a string is `json.loads`-unwrapped up to 2 times; non-dict result → VALIDATION "params must be a JSON object mapping parameter names to values".
4. **Context economy:** `list_models` default `limit` is **20** (was 50); its Field description tells small-context callers to filter with `type`/`query`. Tool docstrings lose their `Args:` sections (now duplicated by Field descriptions); `generate`'s description ≤ ~1,500 chars but MUST retain: the 3-call contract, URLs-only + upload_media pointer, select-values-as-strings, confirm override, TIMEOUT_PENDING resume hint, and the error-key discriminator. All 9 tools annotated `-> dict[str, Any]` (uniform structuredContent/outputSchema).
5. **Transport:** new Settings fields `transport` ("stdio" | "sse" | "streamable-http", env `PIXIO_TRANSPORT`, default "stdio", invalid → VALIDATION naming the var), `host` (`PIXIO_HOST`, default "127.0.0.1"), `port` (`PIXIO_PORT`, default 8000, int-validated). `main()` applies host/port to `mcp.settings` and calls `mcp.run(transport=settings.transport)`. `main()` catches PixioError from `Settings.from_env()` and exits 1 with ONE clean stderr line (no traceback). Tool descriptions are registered dedented (`inspect.getdoc`).
6. **Client:** `download()` also sends Authorization when the URL host equals the configured `base_url` host (covers localhost/LAN gateway overrides; never third-party CDNs). 3xx API responses map to UPSTREAM_ERROR with `details={"location": <Location header>}` and a message hinting to update `PIXIO_BASE_URL`.
7. **media:** `upload_media` detects a `data:` scheme and returns VALIDATION telling the caller to pass data: URLs directly to generate; `download_output` maps un-creatable/`file`-occupied `dest_dir` to VALIDATION (not UPSTREAM_ERROR).
8. **Packaging:** MIT `LICENSE` file at repo root; pyproject gains `license = "MIT"` (SPDX), `[project.urls]` Repository/Issues, and Python 3.11–3.13 classifiers.
